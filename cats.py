...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for i in range(len(sofar)):
        if sofar[i] !=prompt[i]:
            ...
            break
    if t and r==0.0:
        r=(len(sofar))/len(prompt)
    d={'id':user_id,'progress':r}
    upload(d)
    return r
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    for i in range(l2):
        ...
        for j in range(1,len(times_per_player[i])):
            t[i]+=[times_per_player[i][j]-times_per_player[i][j-1]]
    return match(w,t)
    # END PROBLEM 9
